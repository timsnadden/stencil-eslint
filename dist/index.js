...
function isPrivate(originalNode) {
    if (originalNode.modifiers) {
        return originalNode.modifiers.some(m => (m.kind === ts__default["default"].SyntaxKind.PrivateKeyword ||
        ...
        return Object.assign(Object.assign({}, stencil.rules), { 'PropertyDefinition': (node) => {
                ...
                if (stencil.isComponent() && propDecorator) {
                    ...
                    const hasReadonly = !!(originalNode.modifiers &&
                        originalNode.modifiers.some(m => m.kind === ts__default["default"].SyntaxKind.ReadonlyKeyword));
                    if (!hasReadonly) {
                        context.report({
                            node: node.key,
...
function getTypeFailure(type, options) {
    ...
    switch (triState(kind)) {
        case true:
            ...
            return isTypeFlagSet(type, ts__namespace.TypeFlags.Any | ts__namespace.TypeFlags.BooleanLiteral) ? undefined : 0 /* AlwaysTruthy */;
        case false:
            // Allow 'false' itself, but not any other always-falsy type
            return isTypeFlagSet(type, ts__namespace.TypeFlags.BooleanLiteral) ? undefined : 1 /* AlwaysFalsy */;
        case undefined:
            return undefined;
    }
}
...
function getKind(type) {
    return is(ts__namespace.TypeFlags.StringLike) ? 0 /* String */ :
        is(ts__namespace.TypeFlags.NumberLike) ? 2 /* Number */ :
            is(ts__namespace.TypeFlags.Boolean) ? 4 /* Boolean */ :
                isObject('Promise') ? 10 /* Promise */ :
                    is(ts__namespace.TypeFlags.Null) ? 6 /* Null */ :
                        is(ts__namespace.TypeFlags.Undefined | ts__namespace.TypeFlags.Void) ? 7 /* Undefined */
                            :
                                is(ts__namespace.TypeFlags.EnumLike) ? 8 /* Enum */ :
